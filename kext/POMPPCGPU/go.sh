...
#
# Journal complet : /var/log/pomppcgpu-build.log ; résumé sur la console.
LOG=/var/log/pomppcgpu-build.log
SRC=$(dirname "$0")
: > $LOG
...
rm -rf /tmp/qgpu; mkdir -p /tmp/qgpu
cp "$SRC"/*.h "$SRC"/*.cpp "$SRC"/*.c "$SRC"/Info.plist "$SRC"/Makefile /tmp/qgpu/ >> $LOG 2>&1
cd /tmp/qgpu || exit 1
...
say "COMPILATION OK : $(ls -l /tmp/qgpu/POMPPCGPU | awk '{print $5}') octets"

log "=== installation ==="
rm -rf /System/Library/Extensions/POMPPCGPU.kext
cp -R /tmp/qgpu/POMPPCGPU.kext /System/Library/Extensions/ >> $LOG 2>&1
chown -R root:wheel /System/Library/Extensions/POMPPCGPU.kext
chmod -R 755 /System/Library/Extensions/POMPPCGPU.kext
