...
| présentation par l'hôte (21/09/2026, invité, device v15) | Marble Blast fenêtré, bilan `POMPPC_GL_STATS` | `QGPU_CAP_SCANOUT` manquait : `SURF_PRESENT` ne connaissait que la VRAM de `qfb-pci`, absent de la configuration de tous les jours — la v13 n'a jamais servi jusque-là. Cible de repli sur le framebuffer VGA de mac99 : `present` 0 → 229-766 par tranche de 5 s, `readback` 775 → **2-8**, `copie` 0,30 → **0,00 ms**, **+10 % d'images/s** à triangles comparables |
| tampons 16 bits (21/09/2026, invité, device v15) | `gltest` × 13 scènes avec `GLTEST_COLOR16=1`, `run_v15` | drawable RGB1555 + Z de 16 bits **dans le domaine** : témoins bons sous le plugin comme sous Apple, écart à Apple de 8 à 9/255 sur les dégradés (le pas de 1/31) et 0 sur les couleurs pures. A/B sur `game` : 5760 triangles hôte et 1177 img/s, contre **0 triangle et 48 img/s** avec `POMPPC_GL_XFER16=0` |
