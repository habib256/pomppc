...
| `guest/devloop.py` | Boucle de développement dans une VM Tiger allumée : un job (dossier avec `job.sh`) part par une boîte aux lettres sur le disque brut et revient en quelques secondes. `prepare`, `start [--gui]`, `run DOSSIER`, `shot`, `type`, `stop`. Disque : `DEVDISK=` (image raw). |
